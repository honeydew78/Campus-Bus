...
export default function BusSeat2Booking() {
    ...
    useEffect(() => {
        ...
        const fetchSeats = async () => {
            try {
                const response = await axios.get("http://localhost:4000/api/v1/seat2s/get-seat2s"); // Replace with actual endpoint
    ...
    const handleConfirmBooking = async () => {
        if (selectedSeat) {
            try {
                const response = await axios.post("http://localhost:4000/api/v1/seat2s/book-seat2", {
                    seatNumber: selectedSeat
                });
                if (response.status === 200) {
                    setBookedSeats([...bookedSeats, selectedSeat]);
                    navigate('/home-admin/ticket2', { state: { seatNumber: selectedSeat } });
                }
            } catch (error) {
    ...
    return (
      ...
      <div className="w-4/7">
          ...
          <p className="ml-16 text-lg"> <span className="text-lg font-medium">Booking from :</span> Civil Lines to IIITA</p>
          <p className="ml-16 text-lg"> <span className="text-lg font-medium">Timing :</span> 9:00 PM  </p>
          <br />
          ...
          {[...Array(rows * columns)].map((_, index) => {
                        ...
                        const isGapRow = Math.floor(index / columns) === 2;
