...
const CurrentTraineesList = () => {
  ...
  useEffect(() => {
    const fetchTrainees = async () => {
      try {
        const response = await fetch('http://localhost:4000/api/v1/currentTrainees');
        if (!response.ok) {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col justify-center items-center">
      <div className="p-4 rounded shadow-lg w-full max-w-4xl">
        <h2 className="text-xl font-bold mb-4">List of Current Trainees</h2>
        <input
          type="text"
        ...
        {filteredTrainees.length === 0 ? (
          <p>No current trainees found</p>
        ) : (
          <ul className="space-y-2">
            {filteredTrainees.map((trainee) => (
              <li key={trainee._id} className="p-2 border border-gray-300 rounded">
                <Link to={`/home-admin/home-current-trainees/${trainee._id}`} className="text-sm text-green-700 hover:underline">
                  {trainee.fullName}
                </Link>
              </li>
            ))}
...
export default CurrentTraineesList;
