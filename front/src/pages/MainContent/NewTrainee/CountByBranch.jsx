...
const TraineesByBranch = () => {
  ...
  useEffect(() => {
    const fetchTraineesByBranch = async () => {
      try {
        const response = await axios.get('http://localhost:4000/api/v1/newTrainees/stats/count-by-branch');
  ...
  return (
    <div className="bg-white min-h-screen p-6">
      <h1 className="text-3xl font-bold text-orange-600 text-center mb-8">Trainees Count by Branch</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {trainees.map((trainee) => (
          <div key={trainee.branch} className="bg-orange-100 p-6 rounded-lg shadow-md">
            <h2 className="text-xl font-semibold text-orange-800 mb-2">{trainee.branch}</h2>
