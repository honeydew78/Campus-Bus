  ...
  return (
    <div className="bg-white p-6 max-w-md mx-auto rounded shadow-lg">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <RenderField label="Father's Name" value={trainee.fatherName} name="fatherName" editMode={editMode} onChange={handleChange} />
          <RenderField label="Date of Birth" value={trainee.dob} name="dob" editMode={editMode} onChange={handleChange} />
          <RenderField label="Phone" value={trainee.phone} name="phone" editMode={editMode} onChange={handleChange} />
          <RenderField label="City" value={trainee.city} name="city" editMode={editMode} onChange={handleChange} />
          <RenderField label="Branch" value={trainee.branch} name="branch" editMode={editMode} onChange={handleChange} />
          <RenderField label="Time of Join" value={trainee.timeOfJoin} name="timeOfJoin" editMode={editMode} onChange={handleChange} />
        </div>
        <div>
          <RenderField label="Address" value={trainee.address} name="address" editMode={editMode} onChange={handleChange} textarea />
          <RenderField label="Institute" value={trainee.institute} name="institute" editMode={editMode} onChange={handleChange} />
          <RenderField label="Establishment" value={trainee.establishment} name="establishment" editMode={editMode} onChange={handleChange} />
          {editMode ? (
            <>
...
const RenderField = ({ label, value, name, editMode, onChange, textarea }) => {
  return (
    <>
      <p className="font-bold">{label}:</p>
      {editMode ? (
        textarea ? (
          <textarea name={name} value={value} onChange={onChange} className="border rounded p-1 mb-2 w-full" />
        ) : (
          <input type="text" name={name} value={value} onChange={onChange} className="border rounded p-1 mb-2 w-full" />
        )
      ) : (
        <p>{value}</p>
      )}
