...
const InputPage = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post('http://localhost:4000/api/v1/pastTrainees/find-past-trainee', { email, applicationId });
      const traineeId = response.data.data._id;
      setError('');
      navigate(`/home-admin/home-past-trainees/${traineeId}`);
    } catch (err) {
  ...
  return (
    <div className="bg-white p-6 max-w-md mx-auto rounded shadow-lg">
      <h1 className="text-2xl mb-4 text-center text-green-800">Find Past Trainee</h1>
