...
const GetPastTrainee = () => {
  ...
  useEffect(() => {
    const fetchTrainee = async () => {
      try {
        const response = await axios.get(`http://localhost:4000/api/v1/pastTrainees/${id}`);
  ...
  const handleDeleteClick = async () => {
    try {
      if (window.confirm(`Are you sure you want to delete ${trainee.fullName}?`)) {
        await axios.delete(`http://localhost:4000/api/v1/pastTrainees/${id}/delete`);
        navigate('/home-admin/home-past-trainees');
      }
    } catch (err) {
      setError(err.response.data.message || 'An error occurred');
    }
    ...
    <div className="bg-white p-6 max-w-md mx-auto rounded shadow-lg">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <RenderField label="Application ID" value={trainee.applicationId} name="applicationId" editMode={editMode} onChange={handleChange} editedData={editedData} />
          ...
          <RenderField label="End Date" value={trainee.endDate} name="endDate" editMode={editMode} onChange={handleChange} editedData={editedData} />
        </div>
        <div>
